    ...
    def _conv_stage(self, input_tensor, k_size, out_dims, name, group_size=32,
        ...
        with tf.compat.v1.variable_scope(name, reuse = reuse):
            ...
            if group_size:
                ...
                relu = self.relu(inpudata = gn, name = 'relu')
            else:
                relu = self.relu(inpudata = conv, name = 'relu')
    ...
    def _fc_stage(self, input_tensor, out_dims, name, use_bias=False, reuse=False):
        ...
        with tf.compat.v1.variable_scope(name, reuse = reuse):
            fc = self.fullyconnect(inputdata = input_tensor, out_dim = out_dims, use_bias = use_bias,
                                   name = 'fc')
            relu = self.relu(inpudata = fc, name = 'relu')
    ...
    def extract_feats(self, input_tensor, name, reuse=False):
        ...
        with tf.compat.v1.variable_scope(name, reuse = reuse):
            ...
            conv_1_1 = self._conv_stage(input_tensor = input_tensor, k_size = 3,
                                        group_size = 0, out_dims = 64, name = 'conv_1_1')
            ...
            # pool stage 1
            pool1 = self.maxpooling(inpudata = conv_1_2, kernel_size = 2,
            ...
            # pool stage 2
            pool2 = self.maxpooling(inpudata = conv_2_2, kernel_size = 2,
...
if __name__ == '__main__':
    a = tf.compat.v1.placeholder(dtype = tf.float32, shape = [ 1, 256, 256, 3 ], name = 'input')
