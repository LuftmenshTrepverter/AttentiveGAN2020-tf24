...
class CNNBaseModel(object):
    ...
    def conv2d(inputdata, out_channel, kernel_size, padding='SAME',
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            w = tf.compat.v1.get_variable('b', [ out_channel ], initializer = b_init)
            b = None
            ...
            if split == 1:
                conv = tf.nn.conv2d(inputdata, w, stride, padding, data_format = data_format)
            else:
    ...
    @staticmethod
    def relu(inpudata, name=None):
        """
        :param name:
        :param inputdata:
        :return:
        """
        return tf.nn.relu(features = inpudata, name = name)

    @staticmethod
    def sigmoid(inpudata, name=None):
        """
        ...
        :return:
        """
        return tf.nn.sigmoid(x = inpudata, name = name)

    @staticmethod
    def maxpooling(inpudata, kernel_size, strides=None, padding='VALID',
        ...
        return tf.nn.max_pool(value = inpudata, ksize = kernel, strides = strides, padding = padding,
    ...
    def globalavgpooling(inputdata, data_format='NHWC', name=None):
        ...
        return tf.reduce_mean(input_tensor = inputdata, axis = axis, name = None)
    ...
    def layernorm(inputdata, epsilon=1e-5, use_bias=True, use_scale=True,
        ...
        if use_bias:
            beta = tf.compat.v1.get_variable('beta', [ channel ], initializer = tf.constant_initializer())
            beta = tf.reshape(beta, new_shape)
        else:
            beta = tf.zeros([ 1 ] * ndims, name = 'beta')
        if use_scale:
            gamma = tf.compat.v1.get_variable('gamma', [ channel ], initializer = tf.constant_initializer(1.0))
    ...
    def layerbn(inputdata, is_training, name):
        ...
        with tf.compat.v1.variable_scope():
            return tf.compat.v1.layers.batch_normalization(inputs = inputdata, training = is_training)
    ...
    def spatial_dropout(input_tensor, keep_prob, is_training, name, seed=1234):
        ...
        空间dropout实现
        :param input_tensor:
        :param keep_prob:#决定每个元素被保留的概率，(0,1]
        :param is_training:
        :param name:
        :param seed:
        :return:
        ...
        def f2():
            ...
            with tf.compat.v1.variable_scope(name):
                ...
                random_tensor += tf.compat.v1.random_uniform(num_feature_maps,
                                                             seed = seed,
                                                             dtype = input_tensor.dtype)  # 本来默认均匀分布返回值范围为[0,1)，加上keep_prob后返回值范围为[keep_prob，1+keep_prob)
