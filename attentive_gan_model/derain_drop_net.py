...
class DeRainNet(object):
    ...
    def compute_loss(self, input_tensor, gt_label_tensor, mask_label_tensor, name, reuse=False):
        ...
        with tf.compat.v1.variable_scope(name, reuse = reuse):
            ...
            auto_encoder_input = tf.concat((attentive_rnn_output, input_tensor), axis = -1, name = 'autoencoder_input')
...
if __name__ == '__main__':
    ...
    test
    """
    input_tensor = tf.compat.v1.placeholder(dtype = tf.float32, shape = [ 5, 256, 256, 3 ])
    label_tensor = tf.compat.v1.placeholder(dtype = tf.float32, shape = [ 5, 256, 256, 3 ])
    mask_tensor = tf.compat.v1.placeholder(dtype = tf.float32, shape = [ 5, 256, 256, 1 ])
