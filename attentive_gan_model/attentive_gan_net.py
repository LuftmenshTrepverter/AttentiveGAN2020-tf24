...
class GenerativeNet(cnn_basenet.CNNBaseModel):
    ...
    def compute_autoencoder_loss(self, input_tensor, label_tensor, name, reuse=False):
        ...
        label_tensor_resize_2 = tf.compat.v1.image.resize_bilinear(images = label_tensor,
                                                                   size = (int(ori_height / 2), int(ori_width / 2)))
        label_tensor_resize_4 = tf.compat.v1.image.resize_bilinear(images = label_tensor,
        ...
        with tf.compat.v1.variable_scope(name, reuse=reuse):
            ...
            lp_losses = tf.reduce_mean(lp_losses, name='lp_loss')
...
if __name__ == '__main__':
    input_image = tf.compat.v1.placeholder(dtype=tf.float32, shape=[1, 256, 256, 1])
    auto_label_image = tf.compat.v1.placeholder(dtype=tf.float32, shape=[1, 256, 256, 1])
    rnn_label_image = tf.compat.v1.placeholder(dtype=tf.float32, shape=[1, 256, 256, 1])
