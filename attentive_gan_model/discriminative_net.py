...
class DiscriminativeNet(cnn_basenet.CNNBaseModel):
    ...
    def inference(self, input_tensor, name, reuse=False):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            fc_out = self.sigmoid(inpudata = fc_2, name = 'fc_out')
    ...
    def compute_loss(self, input_tensor, label_tensor, attention_map, name, reuse=False):
        ...
        with tf.compat.v1.variable_scope(name, reuse = reuse):
            ...
            entropy_loss = -tf.math.log(fc_out_r) - tf.math.log(-tf.subtract(fc_out_o, tf.constant(1.0, tf.float32)))
...
if __name__ == '__main__':
    input_image = tf.compat.v1.placeholder(dtype=tf.float32, shape=[32, 896, 896, 3])
    attention_map = tf.compat.v1.placeholder(dtype = tf.float32, shape = [ 32, 896, 896, 1 ])
    label_image = tf.compat.v1.placeholder(dtype = tf.float32, shape = [ 32, 896, 896, 3 ])
